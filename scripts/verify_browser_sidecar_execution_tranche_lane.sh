...
TASK_MANIFEST="${1:-browser_tasks/tranche-golem-openclaw-next-execution.json}"
...
printf '# Browser Sidecar Execution Tranche Lane Verify\n'
...
printf '\n## Execution Tranche Task Run\n'
task_output="$("$SCRIPT_DIR/browser_sidecar_execution_tranche_run.sh" "$TASK_MANIFEST" 2>&1)"
printf '%s\n' "$task_output"

printf '\n## Classification\n'
if printf '%s\n' "$task_output" | grep -q 'artifact_kind: browser-sidecar-execution-tranche' && \
   printf '%s\n' "$task_output" | grep -q '## Tranche Selection Matrix' && \
   printf '%s\n' "$task_output" | grep -q '## Final Execution Brief' && \
   printf '%s\n' "$task_output" | grep -q 'EXECUTION_TRANCHE_FINAL_ARTIFACT_JSON' && \
   printf '%s\n' "$task_output" | grep -q 'EXECUTION_TRANCHE_FINAL_ARTIFACT_MD' && \
   printf '%s\n' "$task_output" | grep -q 'selected_tranche:' && \
   printf '%s\n' "$task_output" | grep -q 'runner_up:'; then
  printf 'browser_sidecar_execution_tranche_lane | PASS | selected a single tranche with runner-up, scope, verify and kill criteria\n'
  printf '\nVERIFY_BROWSER_SIDECAR_EXECUTION_TRANCHE_LANE_OK\n'
else
  printf 'browser_sidecar_execution_tranche_lane | FAIL | execution tranche lane did not emit the expected brief markers\n'
  printf '\nVERIFY_BROWSER_SIDECAR_EXECUTION_TRANCHE_LANE_FAIL\n'
  exit 1
fi
