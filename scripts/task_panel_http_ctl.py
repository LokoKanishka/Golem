...
SERVER_SCRIPT = REPO_ROOT / "scripts" / "task_panel_http_server.py"
DEFAULT_HOST = "127.0.0.1"
DEFAULT_PORT = 8765
DEFAULT_PID_FILE = REPO_ROOT / "state" / "tmp" / "task_panel_http_server.pid"
DEFAULT_LOG_FILE = REPO_ROOT / "state" / "tmp" / "task_panel_http_server.log"
DEFAULT_SERVICE_NAME = "golem-task-panel-http.service"
DEFAULT_SERVICE_UNIT_PATH = pathlib.Path.home() / ".config" / "systemd" / "user" / DEFAULT_SERVICE_NAME
SERVICE_TEMPLATE_PATH = REPO_ROOT / "config" / "systemd-user" / "golem-task-panel-http.service.template"

SERVICE_ENV_HOST = "GOLEM_TASK_PANEL_HTTP_HOST"
SERVICE_ENV_PORT = "GOLEM_TASK_PANEL_HTTP_PORT"
...
def fetch_api_summary(host: str, port: int, timeout: float) -> Optional[Dict[str, object]]:
    url = f"http://{host}:{port}/tasks/summary"
    try:
...
def render_service_unit(args: argparse.Namespace) -> str:
    ...
    values = {
        "description": "Golem Task Panel HTTP API",
        ...
        "host_env": SERVICE_ENV_HOST,
        "port_env": SERVICE_ENV_PORT,
        "host": systemd_quote(args.host),
        "port": systemd_quote(str(args.port)),
    }
...
def journalctl_user(args: list[str]) -> subprocess.CompletedProcess[str]:
    return subprocess.run(["journalctl", "--user", *args], capture_output=True, text=True)
...
def wait_for_http_ready(host: str, port: int, timeout: float) -> bool:
    deadline = time.time() + timeout
    while time.time() < deadline:
        if fetch_api_summary(host, port, timeout=1.0) is not None:
            return True
        time.sleep(0.2)
    return False
...
def read_status_snapshot(args: argparse.Namespace) -> Dict[str, object]:
    ...
    summary = fetch_api_summary(args.host, args.port, timeout=args.api_timeout)
    snapshot = {
        "service_mode": False,
        ...
        "api_ready": summary is not None,
        "api_summary_total": summary.get("inventory", {}).get("total") if isinstance(summary, dict) else None,
        "stale_pid": bool(pid and not pid_alive(pid)),
    }
    return snapshot
...
def print_status(snapshot: Dict[str, object], as_json: bool) -> None:
    ...
    print(f"api_running: {'yes' if snapshot.get('running') else 'no'}")
    ...
    print(f"api_summary_total: {snapshot.get('api_summary_total') if snapshot.get('api_summary_total') is not None else '(none)'}")
    print(f"base_url: {snapshot.get('base_url')}")
...
def command_start(args: argparse.Namespace) -> int:
    ...
    pid = read_pid(args.pid_file)
    if pid_alive(pid):
        print(f"task panel http already running with pid {pid}", file=sys.stderr)
        return 1
    ...
    if not wait_for_http_ready(args.host, args.port, args.start_timeout):
        try:
            os.kill(process.pid, signal.SIGTERM)
        except OSError:
            pass
        remove_pid_file(args.pid_file)
        print("FAIL: task panel http did not become ready", file=sys.stderr)
        return 1

    print(f"TASK_PANEL_HTTP_STARTED pid={process.pid} url=http://{args.host}:{args.port}")
    return 0
...
def command_stop(args: argparse.Namespace) -> int:
    ...
    pid = read_pid(args.pid_file)
    if pid <= 0:
        print("TASK_PANEL_HTTP_STOPPED already_not_running")
        return 0
    if not pid_alive(pid):
        remove_pid_file(args.pid_file)
        print("TASK_PANEL_HTTP_STOPPED stale_pid_removed")
        return 0
    ...
    while time.time() < deadline:
        if not pid_alive(pid):
            remove_pid_file(args.pid_file)
            print(f"TASK_PANEL_HTTP_STOPPED pid={pid}")
            return 0
        time.sleep(0.2)
    ...
    while time.time() < deadline:
        if not pid_alive(pid):
            remove_pid_file(args.pid_file)
            print(f"TASK_PANEL_HTTP_STOPPED pid={pid} killed=yes")
            return 0
        time.sleep(0.2)

    print(f"FAIL: task panel http pid {pid} did not stop", file=sys.stderr)
    return 1
...
def command_restart(args: argparse.Namespace) -> int:
    if getattr(args, "service", False):
        ...
        main_pid = int(properties.get("MainPID", "0") or "0")
        if properties.get("ActiveState") != "active" or main_pid <= 0 or not wait_for_http_ready(args.host, args.port, args.start_timeout):
            print("FAIL: task panel http service did not come back after restart", file=sys.stderr)
            return 1
        print(f"TASK_PANEL_HTTP_SERVICE_RESTARTED service={args.service_name} pid={main_pid}")
        return 0
...
def command_service_install(args: argparse.Namespace) -> int:
    ...
    args.service_unit_path.write_text(render_service_unit(args), encoding="utf-8")
    ...
    if args.start_service:
        return command_service_start(args)
    print(f"TASK_PANEL_HTTP_SERVICE_INSTALLED service={args.service_name} unit={args.service_unit_path}")
    return 0
...
def command_service_uninstall(args: argparse.Namespace) -> int:
    ...
    print(f"TASK_PANEL_HTTP_SERVICE_UNINSTALLED service={args.service_name}")
    return 0
...
def command_service_start(args: argparse.Namespace) -> int:
    ...
    main_pid = int(properties.get("MainPID", "0") or "0")
    if properties.get("ActiveState") != "active" or main_pid <= 0 or not wait_for_http_ready(args.host, args.port, args.start_timeout):
        print("FAIL: task panel http service did not become ready", file=sys.stderr)
        return 1
    print(f"TASK_PANEL_HTTP_SERVICE_STARTED service={args.service_name} pid={main_pid}")
    return 0
...
def command_service_stop(args: argparse.Namespace) -> int:
    ...
    if properties.get("ActiveState") != "inactive":
        print("FAIL: task panel http service did not stop cleanly", file=sys.stderr)
        return 1
    print(f"TASK_PANEL_HTTP_SERVICE_STOPPED service={args.service_name}")
    return 0


def command_service_preflight(args: argparse.Namespace) -> int:
    if not args.host:
        print("FAIL: host is required", file=sys.stderr)
        return 1
    if args.port <= 0 or args.port > 65535:
        print("FAIL: port must be between 1 and 65535", file=sys.stderr)
        return 1
    if not os.environ.get("PATH"):
        print("FAIL: PATH is empty", file=sys.stderr)
        return 1
    print("TASK_PANEL_HTTP_SERVICE_PREFLIGHT_OK")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Operational control surface for the local task panel HTTP API.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    def add_common_flags(subparser: argparse.ArgumentParser) -> None:
        subparser.add_argument("--host", default=os.environ.get(SERVICE_ENV_HOST, DEFAULT_HOST))
        subparser.add_argument("--port", type=int, default=int(os.environ.get(SERVICE_ENV_PORT, str(DEFAULT_PORT))))
