...
TASK_MANIFEST="${1:-browser_tasks/decision-reserved-domains-best-source.json}"
...
printf '# Browser Sidecar Decision Lane Verify\n'
...
printf '\n## Decision Task Run\n'
decision_output="$("$SCRIPT_DIR/browser_sidecar_decision_run.sh" "$TASK_MANIFEST" 2>&1)"
printf '%s\n' "$decision_output"

printf '\n## Classification\n'
if printf '%s\n' "$decision_output" | grep -q 'artifact_kind: browser-sidecar-decision' && \
   printf '%s\n' "$decision_output" | grep -q '## Criterion Matrix' && \
   printf '%s\n' "$decision_output" | grep -q '## Final Verdict' && \
   printf '%s\n' "$decision_output" | grep -q 'DECISION_FINAL_ARTIFACT_JSON' && \
   printf '%s\n' "$decision_output" | grep -q 'DECISION_FINAL_ARTIFACT_MD'; then
  printf 'browser_sidecar_decision_lane | PASS | public decision task produced dossier base, criterion matrix and final verdict\n'
  printf '\nVERIFY_BROWSER_SIDECAR_DECISION_LANE_OK\n'
else
  printf 'browser_sidecar_decision_lane | FAIL | decision lane did not emit the expected matrix/verdict markers\n'
  printf '\nVERIFY_BROWSER_SIDECAR_DECISION_LANE_FAIL\n'
  exit 1
fi
