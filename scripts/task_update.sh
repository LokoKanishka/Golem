...
valid_statuses = {"queued", "running", "blocked", "delegated", "worker_running", "done", "failed", "cancelled"}

if new_status not in valid_statuses:
    print(
        "ERROR: status invalido. Usar uno de: blocked, cancelled, delegated, done, failed, queued, running, worker_running",
        file=sys.stderr,
    )
    raise SystemExit(1)
