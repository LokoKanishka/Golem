...
report.append("")
if combined_counts["corrupt"] > 0:
    report.append("La próxima acción correcta es tratar primero los `corrupt` detectados.")
    report.append("")
    report.append("1. inspeccionarlos individualmente;")
    report.append("2. decidir reparación, cuarentena o descarte;")
    report.append("3. recién después continuar con migración legacy.")
    report.append("")
elif combined_counts["legacy"] > 0:
    report.append("La próxima acción correcta es continuar con migración legacy controlada en tandas chicas o moderadas, porque el carril activo ya no tiene `corrupt`.")
    report.append("")
    report.append("1. seguir con batches auditables;")
    report.append("2. validar cada lote en estricto;")
    report.append("3. rerun del baseline después de cada tanda.")
    report.append("")
else:
    report.append("La próxima acción correcta es mantener el inventario sano y pasar a reconciliación con panel/WhatsApp, porque ya no quedan `legacy` ni `corrupt`.")
    report.append("")
    report.append("1. sostener validate/archive;")
    report.append("2. vigilar regresiones;")
    report.append("3. avanzar sobre integración y reconciliación.")
    report.append("")
