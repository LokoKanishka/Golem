...
CANDIDATES="diagnostics/task_audit/legacy_batch_02_candidates.txt"
DRY_RUN_OUT="diagnostics/task_audit/legacy_batch_02_dry_run.txt"
MIGRATED_OUT="diagnostics/task_audit/legacy_batch_02_migrated.txt"
VALIDATE_OUT="diagnostics/task_audit/legacy_batch_02_validate.txt"
...
selected = legacy_paths[:25]
candidates_out.write_text("\n".join(selected) + ("\n" if selected else ""), encoding="utf-8")
print(f"LEGACY_BATCH_02_SELECTED {len(selected)}")
...
[[ "$COUNT" -gt 0 ]] || {
  echo "No legacy candidates found for batch 02." >&2
  exit 3
}
...
echo "LEGACY_BATCH_02_DONE count=$COUNT"
