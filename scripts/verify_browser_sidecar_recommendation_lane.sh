...
TASK_MANIFEST="${1:-browser_tasks/recommend-openclaw-public-baseline.json}"
...
printf '# Browser Sidecar Recommendation Lane Verify\n'
...
printf '\n## Recommendation Task Run\n'
recommendation_output="$("$SCRIPT_DIR/browser_sidecar_recommendation_run.sh" "$TASK_MANIFEST" 2>&1)"
printf '%s\n' "$recommendation_output"

printf '\n## Classification\n'
if printf '%s\n' "$recommendation_output" | grep -q 'artifact_kind: browser-sidecar-recommendation' && \
   printf '%s\n' "$recommendation_output" | grep -q '## Recommendation Matrix' && \
   printf '%s\n' "$recommendation_output" | grep -q '## Final Recommendation' && \
   printf '%s\n' "$recommendation_output" | grep -q 'RECOMMENDATION_FINAL_ARTIFACT_JSON' && \
   printf '%s\n' "$recommendation_output" | grep -q 'RECOMMENDATION_FINAL_ARTIFACT_MD'; then
  printf 'browser_sidecar_recommendation_lane | PASS | public recommendation task produced dossier base, decision base and a final recommendation matrix\n'
  printf '\nVERIFY_BROWSER_SIDECAR_RECOMMENDATION_LANE_OK\n'
else
  printf 'browser_sidecar_recommendation_lane | FAIL | recommendation lane did not emit the expected matrix/recommendation markers\n'
  printf '\nVERIFY_BROWSER_SIDECAR_RECOMMENDATION_LANE_FAIL\n'
  exit 1
fi
