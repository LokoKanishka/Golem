...
DOC="${REPO_ROOT}/docs/OPENCLAW_STATUS_SKELETON_COMPLETION_EXAMPLES.md"
...
[ -f "${DOC}" ] || fail "missing completion examples doc: ${DOC}"
...
assert_file_contains "${DOC}" "## Relacion con skeletons pack"
assert_file_contains "${DOC}" "## Estructura minima de un completion example"
assert_file_contains "${DOC}" "## Completion examples canonicos"
assert_file_contains "${DOC}" "quick-reentry-completion-001"
assert_file_contains "${DOC}" "state-check-completion-001"
...
assert_file_contains "${DOC}" '`explicit_placeholders_to_fill`'
assert_file_contains "${DOC}" "## Artifact y verify por completion example"
assert_file_contains "${DOC}" "## Como convertir un completion example en ticket real"
...
assert_file_contains "${CURRENT_STATE_DOC}" "docs/OPENCLAW_STATUS_SKELETON_COMPLETION_EXAMPLES.md"
assert_file_contains "${HANDOFF_DOC}" "docs/OPENCLAW_STATUS_SKELETON_COMPLETION_EXAMPLES.md"

echo "VERIFY_OK: openclaw status skeleton completion examples checks passed"
