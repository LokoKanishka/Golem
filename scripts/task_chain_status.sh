...
print(f"steps_delegated: {chain_summary.get('steps_delegated', sum(1 for step in steps if step.get('status') == 'delegated'))}")
print(f"steps_running: {chain_summary.get('steps_running', sum(1 for step in steps if step.get('status') in {'running', 'worker_running'}))}")
print(f"steps_skipped: {chain_summary.get('steps_skipped', sum(1 for step in steps if step.get('status') == 'skipped'))}")
print(f"steps_pending: {chain_summary.get('steps_pending', sum(1 for step in steps if step.get('status') in {'planned', 'queued'}))}")
...
print(f"worker_steps_delegated: {chain_summary.get('worker_steps_delegated', 0)}")
print(f"worker_steps_running: {chain_summary.get('worker_steps_running', 0)}")
print(f"children_blocked: {chain_summary.get('children_blocked', 0)}")
print(f"children_delegated: {chain_summary.get('children_delegated', 0)}")
print(f"children_running: {chain_summary.get('children_running', 0)}")
