...
DOC="${REPO_ROOT}/docs/OPENCLAW_STATUS_SEED_INSTANTIATION_EXAMPLES.md"
...
[ -f "${DOC}" ] || fail "missing instantiation pack doc: ${DOC}"
...
assert_file_contains "${DOC}" "## Relacion con seeds pack"
assert_file_contains "${DOC}" "## Estructura minima de una instancia"
assert_file_contains "${DOC}" "## Instanciaciones canonicas"
assert_file_contains "${DOC}" "quick-reentry-instance-001"
assert_file_contains "${DOC}" "state-check-instance-001"
assert_file_contains "${DOC}" "consistency-doc-instance-001"
...
assert_file_contains "${DOC}" "## Artifact y verify por instancia"
assert_file_contains "${DOC}" "## Como convertir una instancia en ticket real"
assert_file_contains "${DOC}" "## Cuando usar estas instancias"
...
assert_file_contains "${CURRENT_STATE_DOC}" "docs/OPENCLAW_STATUS_SEED_INSTANTIATION_EXAMPLES.md"
assert_file_contains "${HANDOFF_DOC}" "docs/OPENCLAW_STATUS_SEED_INSTANTIATION_EXAMPLES.md"

echo "VERIFY_OK: openclaw status seed instantiation examples checks passed"
